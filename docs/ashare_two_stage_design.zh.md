...
2. 预测日志会把当天分数和 ETF 仓位动作保存为可回看的样本，并在后续 1/5/10/20 个交易日逐步回填结果。
...
收益幅度标准化会随周期放大：T+1 使用短线尺度，T+5/T+10/T+20 按周期平方根放宽收益和回撤容忍度，避免中期验证被短线波动尺度过度惩罚。

晨报会展示两行反馈：

```text
昨日验证：前日 3 个强方向命中 2 个；可加仓超额 方向池+0.42% / 上证+0.86%
T+1短线：方向命中率 58%，强信号命中率 64%，方向池+0.18%/日，上证+0.31%/日，loss 0.31
