...
| 20日涨幅 > 12%，或 20日波动率 > 2.5 | `持有观察`，避免高分追涨 |
| score >= 76，当日不转弱，5日涨幅 <= 6%，MA20 偏离 <= 7%，成交额放大在 1.1x-1.8x | `可小幅加仓` |
| score >= 68 | `持有观察` |
| score < 58 | `暂不配置` |
| 其他情况 | `持有观察` |
...
1. `theme_score` 仍然是可解释的规则分，用于表达当日趋势、成交、拥挤度和风格先验。
2. 预测日志会把当天分数和 ETF 仓位动作保存为可回看的样本，并在后续 1/2/5/10/20 个交易日逐步回填结果。

每日运行会维护两个文件：

- `ashare_prediction_log.csv`：逐日记录每个方向的分数、动作、预测信号、T0 价格、上证指数 T0 价格，以及 T+1/T+2/T+5/T+10/T+20 的价格、收益、相对方向池和上证指数的超额收益、命中结果和 loss。
...
验证周期只按真实交易日推进，不按自然日或任务运行日推进。系统从方向 ETF 和上证指数的日 K 线中提取交易日期，只有当预测日和当前行情日期都在交易日列表中，且中间已经经过足够的交易日，才会回填对应的 T+1/T+2/T+5/T+10/T+20 结果。周末、节假日、休市日运行，以及行情源尚未更新到新交易日的运行，都不会提前计入命中率或 loss。
...
后续回填使用双基准。以任意周期 `h` 为例：

```text
actual_hd_return = price_th / price_t0 - 1
actual_excess_universe_hd = 方向ETF h日收益 - 全部方向ETF h日等权平均收益
actual_excess_benchmark_hd = 方向ETF h日收益 - 上证指数 h日收益
actual_excess_hd =
  actual_excess_universe_hd * 65%
+ actual_excess_benchmark_hd * 35%
```

其中方向池口径衡量“方向选择能力”，上证指数口径衡量“是否战胜大盘”。如果上证指数数据暂时缺失，系统会退回只使用方向池口径。
...
每个周期都会单独计算 loss。基础 loss 仍由四个部分组成：
...
收益幅度标准化会随周期放大：T+1/T+2 使用短线尺度，T+5/T+10/T+20 按周期平方根放宽收益和回撤容忍度，避免中期验证被短线波动尺度过度惩罚。对正向信号，T+1/T+2 的 loss 会按 20 日波动率给一个小幅容忍带；小幅跑输或轻微回撤不会被记成完整方向错误，但超过容忍带的下跌、明显跑输和排名落后仍会提高 loss。

晨报会展示两行反馈：

```text
昨日验证：前日 3 个强方向命中 2 个；可加仓超额 方向池+0.42% / 上证+0.86%
T+1/T+2短线：1日命中 58% / 2日命中 61%；强信号 64%，loss 0.31/0.28
T+5/T+10波段：5日命中 62% / 10日命中 59%；T+20主线命中 55%，loss 0.44
```

T+1 主要校准 ETF 仓位动作，T+5/T+10 校准方向持续性，T+20 校准中期主线捕捉能力。这一步暂时只做评估和展示，不直接改写 `theme_score`。后续可以在样本量足够后增加校准层，例如对高拥挤、低成交确认且历史胜率差的信号下调预测分，对右侧初期且历史胜率高的信号上调预测分。

系统同时把参数诊断表写入 `ashare_model_state.json` 的 `parameter_diagnostics` 字段，先用于后台观察，不直接展示在晨报里，也不自动调参。诊断表按以下维度分组：

- ETF 动作：`可小幅加仓`、`持有观察`、`等回调`、`暂不配置`、`减仓提醒` 等。
- 分数段：`<58`、`58-67`、`68-71`、`72+`。
- 过热状态：5 日涨幅 > 8% 或 MA20 偏离 > 10% 记为 `短线过热`，否则为 `非过热`。
- 风格：来自方向池里的 `style` 字段，例如 tech、growth、low risk 等。

每个分组记录样本数、T+1/T+5/T+20 命中率、T+1 相对方向池平均超额和 T+1 平均 loss。等样本足够后，这张缓存表用于判断哪些动作、阈值、过热规则或风格先验需要人工确认后再调参。
...
- `暂缓` 方向只限制追高，不保护低分标的：`score >= 68` 显示为 `B / 不追高`，`52 <= score < 68` 显示为 `C / 持有观察`，`score < 52` 显示为 `D / 减仓/回避`。
